...
# ------------------------------------------------------------------------------
    def dominante(self, choix):
        gamme = 'C#D#EF#G#A#BC#D#EF#G#A#B'                          # Possibilité des notes facilite la suite du code
            ...
            if gamme[i] == choix[0]:                                # Si elle correspond à la note genere precedamment
                ...
                for j in range(len(gamme)):                         # Puis on reparcours la liste des gammes
                    if gamme[j] == "C":                             # Puis la note est DO alors on augmente l'octave
                        octave += 1
                    if len(gamme) % 2 != 0:                         # 2 cas : si impair
                        if j <= (len(gamme) // 2):                  # Pour les probabilite
                            for k in range(j+1):                    # On copie j fois la note dans la liste
                                if gamme[j] == '#':
                                    self.__notes.append(str(self.__notes[-1][0])+str(gamme[j])+str(octave))
                                else:
                        ...
                        if j <= (len(gamme) //2)-1:                 # Pour les probabilite
                            for k in range(j+1):                    # On copie j fois la note dans la liste
                                if gamme[j] == '#':
                                    self.__notes.append(str(self.__notes[-1][0])+str(gamme[j])+str(octave))
                                else:
                            ...
                            for k in range(j):                      # On copie j fois la note dans la liste
                                if gamme[j] == '#':
                                    self.__notes.append(str(self.__notes[-1][0])+str(gamme[j])+str(octave))
                                else:
                                    self.__notes.append(str(gamme[j])+str(octave))
